function [out_times,freq,all_Cxy_timecourse, trial_numbers, perCorr_pertr, which_event]=drgGetLFPimgCohShuffledForThisEvTypeNo(handles)
...
%If the event is odor on shuffle assigning S+ trials to S+ and S- to S-
for trNo=firstTr:lastTr
    handles.evTypeNo=this_evTypeNo;
    this_evNo = drgFindEvNo(handles,trNo,sessionNo);
    if this_evNo~=-1
        ii=ii+1;
        evNo1(ii)=this_evNo;
        %Now go up and down to find an event of the same type
        handles.evTypeNo=5; %S+
        this_evNoSP = drgFindEvNo(handles,trNo,sessionNo);
        if this_evNoSP==-1
            %This is an S-
            handles.evTypeNo=11;
        end
        ...
        while found_trial==0
            %Try moving up
            if trNo+delta_ii<=lastTr
                this_evNo = drgFindEvNo(handles,trNo+delta_ii,sessionNo);
                if this_evNo~=-1
                    %Found it!
                    handles.evTypeNo=this_evTypeNo;
                    evNo2(ii)=drgFindEvNo(handles,trNo+delta_ii,sessionNo);
                    found_trial=1;
                end
            end
            %Try moving down
            if found_trial==0
                if trNo-delta_ii>=firstTr
                    this_evNo = drgFindEvNo(handles,trNo-delta_ii,sessionNo);
                    if this_evNo~=-1
                        %Found it!
                        handles.evTypeNo=this_evTypeNo;
...
for ii=1:length(evNo1)
    ...
    [LFP1, trialNo, can_read1] = drgGetTrialLFPData(handles, handles.peakLFPNo, evNo1(ii), handles.evTypeNo, min_t, max_t);
    [LFP2, trialNo, can_read2] = drgGetTrialLFPData(handles, handles.burstLFPNo, evNo2(ii), handles.evTypeNo, min_t, max_t);
