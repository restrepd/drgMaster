...
for filNum=first_file:handles.drgbchoices.no_files
    ...
    for winNo=1:handles.drgbchoices.noWindows
        ...
        handles.time_end=handles.drgbchoices.timeEnd(winNo)+handles.time_pad; 
        
        
        %Now run the analysis for each unit pair
        for unitNo=1:handles.drg.session(handles.sessionNo).noUnits
            ...
            for unitNo2=1:handles.drg.session(handles.sessionNo).noUnits
                handles.unitNo2=unitNo2;
                if handles.unitNo~=handles.unitNo2
                    handles.drgb.corrpair_no=handles.drgb.corrpair_no+1;
                    handles=drgPlotCrossCorr(handles);
                    handles.drgb.corrpair(handles.drgb.corrpair_no).groupNo=handles.drgbchoices.group_no(filNum);
                    fprintf(1, 'File number: %d, window number: %d, reference unit: %d, partner unit: %d\n',filNum,winNo,unitNo,unitNo2);
                end
            end
        end
