        ...
        for trNum=1:handles.drgb.PAC.no_trials
            ...
            while at_end==0
                ...
                if (~isempty(ii_next))&(ii+ii_next-1<=length(t_apt))&(ii+ii_next-1<=length(out_times_env))
                    ...
                    jj_cont=jj_cont+1;
                    cont_peakPower(jj_cont)=this_peakPower(jj);
                    t_cont_peakPower(jj_cont)=this_peakPower_times(jj)+handles.drg.draq_d.t_trial(trNum);
        ...
        for trNum=1:handles.drgb.PAC.no_trials
            ...
            while at_end==0
                ...
                if (~isempty(ii_next))&(ii+ii_next-1<=length(t_apt))&(ii+ii_next-1<=length(out_times_env))
                    ...
                    jj_cont=jj_cont+1;
                    cont_troughPower(jj_cont)=this_troughPower(jj);
                    t_cont_troughPower(jj_cont)=this_troughPower_times(jj)+handles.drg.draq_d.t_trial(trNum);
