function drgAnalyzeLFPDiscriminantLaser
...
genotype(2).handles_out=handles_out;
...
for PACii=1:length(handles_out.drgbchoices.PACburstLowF)
    ...
    for genotype_ii=1:2
        ...
        for groupNo=1:max(handles_out.drgbchoices.group_no)
                ...
                try
                    if genotype(genotype_ii).handles_out.discriminant_PACwavepower(mouseNo).group(groupNo).which_analysis(which_analysis).discriminant_calculated==1
                        ...
                        if (per_ii>=20)&(sum(no_mice==mice_excluded)==0)
                            ...
                            all_discriminant_correct_peak(no_mice_included,:)=genotype(genotype_ii).handles_out.discriminant_PACwavepower(mouseNo).group(groupNo).which_analysis(which_analysis).PACii(PACii).discriminant_correct_peak;
                            all_discriminant_correct_trough(no_mice_included,:)=genotype(genotype_ii).handles_out.discriminant_PACwavepower(mouseNo).group(groupNo).which_analysis(which_analysis).PACii(PACii).discriminant_correct_trough;
                            all_discriminant_correct_shuffled_peak(no_mice_included,:)=genotype(genotype_ii).handles_out.discriminant_PACwavepower(mouseNo).group(groupNo).which_analysis(which_analysis).PACii(PACii).discriminant_correct_shuffled_peak;
                            all_discriminant_correct_shuffled_trough(no_mice_included,:)=genotype(genotype_ii).handles_out.discriminant_PACwavepower(mouseNo).group(groupNo).which_analysis(which_analysis).PACii(PACii).discriminant_correct_shuffled_trough;
                            all_dimensionality_peak(no_mice_included,:)=genotype(genotype_ii).handles_out.discriminant_PACwavepower(mouseNo).group(groupNo).which_analysis(which_analysis).PACii(PACii).dimensionality_peak;
                            all_dimensionality_trough(no_mice_included,:)=genotype(genotype_ii).handles_out.discriminant_PACwavepower(mouseNo).group(groupNo).which_analysis(which_analysis).PACii(PACii).dimensionality_trough;
                        end
                    end
                catch
                end
            ...
            no_mice_per(genotype_ii)=no_mice_included;
            ...
            p_correct_stats(ii_stats).description=[handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{genotype_ii} ' shuffled peak'];
            p_correct_stats(ii_stats).data_ii=1;
            p_correct_stats(ii_stats).PACii=PACii;
            p_correct_stats(ii_stats).per_corr_ii=genotype_ii;
            ...
            glm_correct.perCorr(glm_ii+1:glm_ii+length(data))=genotype_ii;
            ...
            ii_stats=ii_stats+1;
            p_correct_stats(ii_stats).data=data;
            p_correct_stats(ii_stats).description=[handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{genotype_ii} ' shuffled trough'];
            ...
            ii_stats=ii_stats+1;
            p_correct_stats(ii_stats).data=data;
            p_correct_stats(ii_stats).description=[handles_out.drgbchoices.group_no_names{groupNo} ' ' genotype_lab{genotype_ii} ' trough'];
            ...
            title(['LDA for theta/' handles_out.drgbchoices.PACnames{PACii} ' ' handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{genotype_ii}])
            
            xlabel('Time (sec)')
            ylabel(['% correct '  genotype_lab{genotype_ii}])
            ...
            title(['Area under the curve for theta/' handles_out.drgbchoices.PACnames{PACii} ' ' handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{genotype_ii}])
...
%                         genotype_lab{percent_correct_ii} ' pre-odor, trough'];
%                 else
%                     dim_stats(ii_dim_stats).description=[handles_out.drgbchoices.group_no_names{groupNo} ' ' ...
%                         genotype_lab{percent_correct_ii} ' odor, trough'];
%                 end
...
%                 if winNo==1
%                     dim_stats(ii_dim_stats).description=[handles_out.drgbchoices.group_no_names{groupNo} ' ' ...
%                         genotype_lab{percent_correct_ii} ' pre-odor, peak'];
%                 else
%                     dim_stats(ii_dim_stats).description=[handles_out.drgbchoices.group_no_names{groupNo} ' ' ...
%                         genotype_lab{percent_correct_ii} ' odor, peak'];
%                 end
%                 dim_stats(ii_dim_stats).winNo=winNo;
%                 dim_stats(ii_dim_stats).PACii=PACii;
%                 dim_stats(ii_dim_stats).per_corr_ii=percent_correct_ii;
...
%             
%             title(['Dimensionality for theta/' handles_out.drgbchoices.PACnames{PACii} ' ' handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{percent_correct_ii}])
%             legend([hltrough hlpeak],{'Trough','Peak'})
%             xlabel('Time (sec)')
%             ylabel(['Dimensionality '  genotype_lab{percent_correct_ii}])
%             
    ...
    tbl = table(glm_correct.data',glm_correct.group',glm_correct.perCorr',glm_correct.shuffled',glm_correct.peak',...
        'VariableNames',{'prediction','laser','genotype','shuffled','peak_trough'});
    mdl = fitglm(tbl,'prediction~laser+genotype+shuffled+peak_trough+peak_trough*shuffled*laser*genotype'...
...
%This is the analysis for percent correct when trained before laser
%and quearied during laser
which_analysis=2;
...
%             
            ...
            ii_stats=ii_stats+1;
            p_correct_stats(ii_stats).data=data;
            p_correct_stats(ii_stats).description=[genotype_lab{genotype_ii} ' trough'];
            p_correct_stats(ii_stats).data_ii=3;
            p_correct_stats(ii_stats).PACii=PACii;
            p_correct_stats(ii_stats).per_corr_ii=genotype_ii;
            p_correct_stats(ii_stats).groupNo=groupNo;
            ...
            ii_stats=ii_stats+1;
            p_correct_stats(ii_stats).data=data;
            p_correct_stats(ii_stats).description=[genotype_lab{genotype_ii} ' peak'];
            p_correct_stats(ii_stats).data_ii=4;
            p_correct_stats(ii_stats).PACii=PACii;
            p_correct_stats(ii_stats).per_corr_ii=genotype_ii;
            p_correct_stats(ii_stats).groupNo=groupNo;
            ...
            title(['LDA trained before laser for theta/' handles_out.drgbchoices.PACnames{PACii} ' ' handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{genotype_ii}])
...
%             plot(p_correct_stats(ii_stats-2).data,p_correct_stats(ii_stats-3).data,'ok')
            ...
            xlabel('AUC trough')
%             legend('Original','Shuffled')
            title(['Area under the curve trained before the laser for theta/' handles_out.drgbchoices.PACnames{PACii} ' ' handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{genotype_ii}])
...
%             
    ...
    %Perform the glm
    fprintf(1, ['\n\nglm for area under the curve tranied before the laser for Theta/' handles_out.drgbchoices.PACnames{PACii} '\n'])
    tbl = table(glm_correct.data',glm_correct.perCorr',glm_correct.peak',...
        'VariableNames',{'prediction','genotype','peak_trough'});
    mdl = fitglm(tbl,'prediction~genotype+peak_trough+peak_trough*genotype'...
        ,'CategoricalVars',[2,3])
...
%                 title(['PC1 peak for Theta/' handles_out.drgbchoices.PACnames{pca_ii} ' ' genotype_lab{percent_correct_ii} ' ' handles_out.drgbchoices.group_no_names{groupNo}])
%                 xlabel('Time (sec)')
%                 ylabel('PC1 peak')
%             end
%             
...
%                 text(-1,minPC1+0.9*(maxPC1-minPC1),handles_out.drg.draq_d.eventlabels{handles_out.drgbchoices.events_to_discriminate(1)},'Color','r')
%                 text(-1,minPC1+0.8*(maxPC1-minPC1),handles_out.drg.draq_d.eventlabels{handles_out.drgbchoices.events_to_discriminate(2)},'Color','b')
%                 title(['PC1 trough for Theta/' handles_out.drgbchoices.PACnames{pca_ii} ' ' genotype_lab{percent_correct_ii} ' ' handles_out.drgbchoices.group_no_names{groupNo}])
%                 xlabel('Time (sec)')
%                 ylabel('PC1 trough')
%             end
%             
...
%         end
%     end
%     suptitle(['All trials. Group: ' handles_out.drgbchoices.group_no_names{groupNo} ' # of mice: ' num2str(no_mice_per(1)) ' ' genotype_lab{1} ' ' num2str(no_mice_per(2)) ' ' genotype_lab{2}])
% end
% 
...
%             
%             
%             title(['p value for Theta/' handles_out.drgbchoices.PACnames{PACii} ' ' handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{percent_correct_ii}])
%             
%             xlabel('Time (sec)')
%             ylabel(['p value '  genotype_lab{percent_correct_ii}])
%             
...
%             
%             %Do ranksum/t test
%             fprintf(1, ['\n\nRanksum or t-test p values for area under the curve for Theta/' handles_out.drgbchoices.PACnames{PACii} ' ' handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{percent_correct_ii} '\n'])
%             try
...
%             
%             title(['Decision times for Theta/' handles_out.drgbchoices.PACnames{PACii} ' ' handles_out.drgbchoices.group_no_names{groupNo}  ' ' genotype_lab{percent_correct_ii}])
