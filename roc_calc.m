function ROCout=roc_calc(varargin)
% Note: I changed the name to roc_calc because there is a "roc" function in
% MATLAB Diego Restrepo
...
%Now calculate the rawArea by calculating the area under the curve using a
%trapezoid integral of the actual data, not the fit!
xyroc=[xroc yroc];
%Add start and end
if isempty(find(xyroc==[0 0]))
    xyroc(end+1,:)=[0 0];
end
if isempty(find(xyroc==[1 1]))
    xyroc(end+1,:)=[1 1];
end

xyroc_sorted=sort(xyroc);
rawArea=trapz(xyroc_sorted(:,1),xyroc_sorted(:,2)); %estimate the area under the curve using the actual data

%standard error of area
rawArea2=rawArea^2; Q1=rawArea/(2-rawArea); Q2=2*rawArea2/(1+rawArea);
V=(rawArea*(1-rawArea)+(lu-1)*(Q1-rawArea2)+(lh-1)*(Q2-rawArea2))/(lu*lh);
Serrorraw=realsqrt(V);

%confidence interval
ci_raw=rawArea+[-1 1].*(realsqrt(2)*erfcinv(alpha)*Serror);
if ci_raw(1)<0; ci_raw(1)=0; end
if ci_raw(2)>1; ci_raw(2)=1; end
m=zeros(1,4); 

%z-test
SAUCraw=(rawArea-0.5)/Serrorraw; %standardized area
p_raw=1-0.5*erfc(-SAUCraw/realsqrt(2)); %p-value


if verbose
    
    %Performance of the classifier using fit
    if Area==1
        str='Perfect test';
    ...
    %display results
    disp('ROC CURVE ANALYSIS using fit data')
    disp(' ')
    disp(tr)
    ...
    %Performance of the classifier using actual data
    if rawArea==1
        str='Perfect test';
    elseif rawArea>=0.90 && Area<1
        str='Excellent test';
    elseif rawArea>=0.80 && Area<0.90
        str='Good test';
    elseif rawArea>=0.70 && Area<0.80
        str='Fair test';
    elseif rawArea>=0.60 && Area<0.70
        str='Poor test';
    elseif rawArea>=0.50 && Area<0.60
        str='Fail test';
    else
        str='Failed test - less than chance';
    end
    ...
    %display results
    disp('ROC CURVE ANALYSIS using actual data')
    disp(' ')
    disp(tr)
    str2=['AUC\t\t\tS.E.\t\t\t\t' num2str((1-alpha)*100) '%% C.I.\t\t\tComment\n'];
    fprintf(str2)
    disp(tr)
    fprintf('%0.5f\t\t\t%0.5f\t\t\t%0.5f\t\t%0.5f\t\t\t%s\n',rawArea,Serrorraw,ci_raw,str)
    disp(tr)
    fprintf('Standardized AUC\t\t1-tail p-value\n')
    if p<1e-4
        fprintf('%0.4f\t\t\t\t%0.4e',SAUCraw,p_raw)
    else
        fprintf('%0.4f\t\t\t\t%0.4f',SAUCraw,p_raw)
    end
    if p<=alpha
        fprintf('\t\tThe area is statistically greater than 0.5\n')
    else
    ...
    plot(xfit,yfit,'marker','none','linestyle','-','color','r','linewidth',3);
...
if nargout
    ROCout.AUC=Area; %Area under the curve calculated from the fit
    ROCout.rawAUC=rawArea;  %Area under the curve calculated from the raw data (added by Diego)
    ...
    ROCout.xf=xfit; %graphic x points
    ROCout.yf=yfit; %graphic y points
    ROCout.table=table;
    ROCout.p=p;
    ROCout.p_raw=p_raw;
end