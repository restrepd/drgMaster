function [output_data] = drgPairsRanksumorTtest(input_data,fileID,inhibit_print)
% This function performs t tests or ranksum for every two data sets in a
% list
...
is_first=1;
output_data.no_significant_pFDR=sum(pvals<output_data.pFDR);
output_data.no_significant=sum(pvals<0.05);
fprintf(1, ['\n' num2str(output_data.no_significant_pFDR) ' out of ' num2str(length(pvals)) ' p values are below pFDR\n']);
fprintf(1, [num2str(output_data.no_significant) ' out of ' num2str(length(pvals)) ' p values are below 0.05\n\n']);
