    ...
    for mouseNo=1:max(handles.drgbchoices.mouse_no)
        ...
        for groupNo=1:max(handles.drgbchoices.group_no)
            ...
            if overwrite_out_file==1
                ...
                if (sum(handles.drgbchoices.which_discriminant==15)>0)
                    try
                        dcalc=handles_out.all_mouse_wav(mouseNo).group;
                        process_data_for_mouse=0;
                    catch
                        process_data_for_mouse=1;
                    end
            ...
            if process_data_for_mouse==1
                ...
                if mouse_has_data==1
                    ...
                    if (sum(handles.drgbchoices.which_discriminant==15)>0)||(sum(handles.drgbchoices.which_discriminant==16)>0)
                        handles_out.all_mouse_wav(mouseNo).group(groupNo).mouse_has_data=mouse_has_data;
                        if mouse_has_data==1
                            handles_out.all_mouse_wav(mouseNo).group(groupNo).t=t_pac;
                            handles_out.all_mouse_wav(mouseNo).group(groupNo).all_perCorr_pertrPACwave=all_perCorr_pertrPACwave;
                            handles_out.all_mouse_wav(mouseNo).group(groupNo).all_licks_per_tPACwave=all_licks_per_tPACwave;
                            handles_out.all_mouse_wav(mouseNo).group(groupNo).all_stamped_lick_iiPACwave=all_stamped_lick_iiPACwave;
                            handles_out.all_mouse_wav(mouseNo).group(groupNo).all_log_P_timecoursePACwavepeak=all_log_P_timecoursePACwavepeak;
                            handles_out.all_mouse_wav(mouseNo).group(groupNo).all_log_P_timecoursePACwavetrough=all_log_P_timecoursePACwavetrough;
                            handles_out.all_mouse_wav(mouseNo).group(groupNo).all_which_eventsPACwave=all_which_eventsPACwave;
                        end
                    end
        ...
        if process_data_for_mouse==1
            if (mouse_has_data==1)&((sum(handles.drgbchoices.which_discriminant==13)>0)||(sum(handles.drgbchoices.which_discriminant==14)>0))
