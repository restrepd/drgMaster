function drgSummaryBatchzscorecase2CaMKII
...
fileID = fopen([PathName 'drgSummaryBatchzscorecase2CaMKII.txt'],'w');
...
for pacii=[1 2]    %for amplitude bandwidths (beta, high gamma)
    ...
    for grNo=1:3
        ...
        input_dect_licks_data(id_dect_licks_ii).description=[group_legend{grNo}];
        ...
        glm_dect.group(glm_dect_ii+1:glm_dect_ii+length(these_data))=(grNo-1)*ones(1,length(these_data));
        glm_dect_ii=glm_dect_ii+length(these_data);
        
        id_dect_ii=id_dect_ii+1;
        input_dect_data(id_dect_ii).data=these_data;
        input_dect_data(id_dect_ii).description=['Peak hippocampus ' group_legend{grNo}];
        ...
        id_dect_ii=id_dect_ii+1;
        input_dect_data(id_dect_ii).data=these_data;
        input_dect_data(id_dect_ii).description=['Trough hippocampus ' group_legend{grNo}];
        
        %Now let's plot decision times for licks
        edges=[0:0.033:0.5];
        rand_offset=0.8;
        ...
        if pacii==1
            %Licks
            all_decision_dt_licks=lick_decision_times_hipp(logical(found_lick_decision_times_hipp));
        ...
        %Do peak and trough PRP
        figure(figureNo_peak_trough)
        hold on
        
        
        %Peak PRP
        all_decision_dt_peak=peak_decision_times_hipp(logical(found_peak_decision_times_hipp));
        bar_offset=grNo;
        switch grNo
            case 1
        ...
        %Trough PRP
        all_decision_dt_trough=trough_decision_times_hipp(logical(found_trough_decision_times_hipp));
        bar_offset=grNo+4;
        switch grNo
            case 1
    ...
    title(['Decision time ' bandwidth_names{pacii} ' hippocampus'])
    ylim([0 0.8])
    ylabel('dt')
...
%         drgWriteANOVANtbl(anovanTbl,fileID);
    ...
    tbl = table(glm_dect.data',glm_dect.lick_peak_trough',glm_dect.group',...
        'VariableNames',{'detection_time','peak_trough','genotype'});
    mdl = fitglm(tbl,'detection_time~peak_trough+genotype+peak_trough*genotype'...
        ,'CategoricalVars',[2 3])
...
%         ylabel('log10(p)')
        ...
        glm_dect.data(glm_dect_ii+1:glm_dect_ii+length(these_data))=these_data;
        glm_dect.peak_trough(glm_dect_ii+1:glm_dect_ii+length(these_data))=zeros(1,length(these_data));
        glm_dect.group(glm_dect_ii+1:glm_dect_ii+length(these_data))=(grNo-1)*ones(1,length(these_data));
        glm_dect_ii=glm_dect_ii+length(these_data);
        
        id_dect_ii=id_dect_ii+1;
        input_dect_data(id_dect_ii).data=these_data;
        input_dect_data(id_dect_ii).description=['Peak prefrontal ' group_legend{grNo}];
        
        %trough
        these_data=trough_decision_times_pre(found_trough_decision_times_pre==1);
        glm_dect.data(glm_dect_ii+1:glm_dect_ii+length(these_data))=these_data;
        glm_dect.peak_trough(glm_dect_ii+1:glm_dect_ii+length(these_data))=ones(1,length(these_data));
        glm_dect.group(glm_dect_ii+1:glm_dect_ii+length(these_data))=(grNo-1)*ones(1,length(these_data));
        glm_dect_ii=glm_dect_ii+length(these_data);
        
        id_dect_ii=id_dect_ii+1;
        input_dect_data(id_dect_ii).data=these_data;
        input_dect_data(id_dect_ii).description=['Trough prefrontal ' group_legend{grNo}];
        ...
        %Peak PRP
        all_decision_dt_peak=peak_decision_times_pre(logical(found_peak_decision_times_pre));
        bar_offset=grNo;
        switch grNo
            case 1
                bar(bar_offset,mean(all_decision_dt_peak),'g','LineWidth', 3,'EdgeColor','none')
        ...
        %Trough PRP
        all_decision_dt_trough=trough_decision_times_pre(logical(found_trough_decision_times_pre));
        bar_offset=grNo+4;
        switch grNo
            case 1
                bar(bar_offset,mean(all_decision_dt_trough),'g','LineWidth', 3,'EdgeColor','none')
...
%             ,edges,bar_offset,rand_offset,'k','k',3);
    ...
    title(['Decision time ' bandwidth_names{pacii} ' prefrontal'])
    ylim([0 0.8])
    ylabel('dt')
    ...
    tbl = table(glm_dect.data',glm_dect.peak_trough',glm_dect.group',...
        'VariableNames',{'detection_time','peak_trough','genotype'});
    mdl = fitglm(tbl,'detection_time~peak_trough+genotype+peak_trough*genotype'...
        ,'CategoricalVars',[2 3])
