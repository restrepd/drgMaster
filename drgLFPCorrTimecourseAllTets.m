function [handles, handles_corr_out]=drgLFPCorrTimecourseAllTets(handles)
 

handles_displayData=handles.displayData;

%Regardless do not display the data when you call drgLFPCorrTimecourse
handles.displayData=0;
handles.save_drgb=1;
handles_corr_out=[];

if ispc
    handles.tempPath=[handles.jtPathName '\temp_corr_' handles.jtFileName(9:end) '\'];
else
    handles.tempPath=[handles.jtPathName '/temp_corr_' handles.jtFileName(9:end) '/'];
end
...
if (exist([handles.jtPathName 'Corr_' handles.jtFileName(9:end)])~=2)||...
    ...
    %In order to save time we are doing each reference electrode compared to one
    %of the other electrodes
    for ii=1:length(handles.drgbchoices.reference_electrodes)
        ref_tet_electNo=handles.drgbchoices.reference_electrodes(ii);
        handles.burstLFPNo=ref_tet_electNo;
        other_tet_electNo=handles.drgbchoices.other_electrodes(ii);
...
handles.displayData=handles_displayData;
...
if handles.displayData==1
    ...
    for ii_pcorr=1:2
        for ii_spm=1:2
            ...
            mean_all_max_rho_t_lag=mean(all_mean_max_rho_t_lag,1);
            if sum(isnan(mean_all_max_rho_t_lag))==0
